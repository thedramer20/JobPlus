...
import "../styles/network-page-v3.css";
            ...
            {suggestedConnectionsData.map((connection) => (
              <div key={connection.id} className="jp-connection-card">
                ...
                <div className="jp-connection-header">
                  <div className="jp-avatar jp-avatar-compact">
                    {connection.avatar ? (
                ...
                {connection.skillOverlap && connection.skillOverlap.length > 0 && (
                  <div className="jp-connection-skills">
                    {connection.skillOverlap.map((skill, idx) => (
                      <span key={idx} className="jp-skill-chip">
                        {skill}
                      </span>
                    ))}
                  </div>
                ...
                {connection.sharedCompanies && connection.sharedCompanies.length > 0 && (
                  <div className="jp-connection-shared">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M4.5 20V7a2 2 0 0 1 2-2h6v15"/>
                      <path d="M12.5 20v-8.7a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2V20"/>
                    </svg>
                    <span>Shared: {connection.sharedCompanies.join(", ")}</span>
                  </div>
                )}

                {/* Actions */}
                <div className="jp-connection-actions">
                  <button className="jp-btn jp-btn-primary jp-btn-compact">
                    Connect
                  </button>
                  <button className="jp-btn jp-btn-secondary jp-btn-compact">
                    View Profile
                  </button>
                </div>
              </div>

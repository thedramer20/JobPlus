...
export function AdminUsersPage() {
  ...
  return (
    <section className="surface jp-admin-section">
      <div className="jp-admin-page-header">
        <div>
        ...
        <div className="jp-admin-page-actions">
          <button className="btn btn-secondary">Export CSV</button>
          <button className="btn btn-secondary" disabled={selectedUsers.length === 0 || loadingAction === "suspend"} onClick={handleBulkSuspend}>
      ...
      <div className="jp-admin-user-filters">
        <label className="jp-admin-filter-field">
          <span>Search</span>
          <input className="input" placeholder="Name, username, or email" value={query} onChange={(event) => setQuery(event.target.value)} />
        </label>
        <label className="jp-admin-filter-field">
          <span>Role</span>
      ...
      <div className="jp-admin-selection-row">
      ...
      <div className="jp-admin-table-wrap">
        <table className="table" style={{ marginTop: "0.5rem" }}>
          <thead>
            <tr>
              <th style={{ width: "40px" }}>
                <input type="checkbox" checked={selectedUsers.length === filtered.length && filtered.length > 0} onChange={(e) => (e.target.checked ? selectAll() : clearSelection())} />
              </th>
              <th>User</th>
              <th>Email</th>
              <th>Role</th>
          ...
          <tbody>
            {filtered.map((item) => (
              <tr key={item.id} className={selectedUsers.includes(item.id) ? "selected" : ""}>
                <td>
                  <input type="checkbox" checked={selectedUsers.includes(item.id)} onChange={() => toggleUserSelection(item.id)} />
                </td>
                <td>
                ...
                <td>{item.email}</td>
                <td>
                  <span className={`status status-${item.role === "admin" ? "danger" : item.role === "employer" ? "warning" : "info"}`}>{item.role}</span>
                </td>
                <td>
                  <span className={`status status-${item.status === "active" ? "success" : item.status === "inactive" ? "warning" : "danger"}`}>{item.status}</span>
                </td>
                <td>
                  <span className={`status status-${item.verification === "verified" ? "success" : item.verification === "pending" ? "warning" : "danger"}`}>{item.verification}</span>
                </td>
                <td>{item.country}</td>
                <td>
                  <span className={`status status-${item.riskLevel === "low" ? "success" : item.riskLevel === "medium" ? "warning" : "danger"}`}>{item.riskLevel}</span>
                </td>
                <td>{item.premium ? "✓" : "—"}</td>
                ...
                <td>
                  <div className="row" style={{ gap: "0.5rem" }}>
                    <button className="btn btn-ghost btn-sm" onClick={() => handleUserAction(item.id, "view")} disabled={loadingAction === `view-${item.id}`}>
                      {loadingAction === `view-${item.id}` ? "..." : "View"}
                    </button>
                    {item.status === "active" ? (
                      <button className="btn btn-secondary btn-sm" onClick={() => handleUserAction(item.id, "suspend")} disabled={loadingAction === `suspend-${item.id}`}>
                        {loadingAction === `suspend-${item.id}` ? "..." : "Suspend"}
                      </button>
                    ) : (
                      <button className="btn btn-success btn-sm" onClick={() => handleUserAction(item.id, "activate")} disabled={loadingAction === `activate-${item.id}`}>
